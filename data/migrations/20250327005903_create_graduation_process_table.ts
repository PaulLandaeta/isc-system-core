...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable(gradprocTable, (table) => {
    ...
    table.boolean('tutor_letter').notNullable();
    table.integer('tutor_id').unsigned().nullable().references('id').inTable('professors');
    table.boolean('tutor_approval').notNullable();
    table.timestamp('date_tutor_assignament', { precision: 6 }).nullable();
    table.boolean('reviewer_letter').notNullable();
    table.integer('reviewer_id').unsigned().notNullable().references('id').inTable('professors');
    table.boolean('reviewer_approval').notNullable();
